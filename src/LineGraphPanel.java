...
public class LineGraphPanel extends JPanel {
    ...
    public LineGraphPanel(HashMap<Integer, Double> data) {
        this.data = data;
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        double xScale = ((double) getWidth() - (2 * padding) - labelPadding) / (data.keySet().size() - 1);
        double yScale = ((double) getHeight() - 2 * padding - labelPadding) / (getMaxScore() - getMinScore());

        List<Point> graphPoints = new ArrayList<>();
        for (Integer xVal : data.keySet()) {
            int x1 = (int) (xVal * xScale + padding + labelPadding);
            int y1 = (int) ((getMaxScore() - data.get(xVal)) * yScale + padding);
        ...
        for (int i = 0; i < data.keySet().size(); i++) {
            if (data.keySet().size() > 1) {
                int x0 = i * (getWidth() - padding * 2 - labelPadding) / (data.keySet().size() - 1) + padding
                        + labelPadding;
                int x1 = x0;
                int y0 = getHeight() - padding - labelPadding;
                int y1 = y0 - pointWidth;
                if ((i % ((int) ((data.keySet().size() / 20.0)) + 1)) == 0) {
                    g2.setColor(gridColor);
